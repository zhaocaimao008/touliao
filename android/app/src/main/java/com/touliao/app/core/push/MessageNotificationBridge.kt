 ...
 *   · socket 断开（离线）→ 服务端发 FCM → 由 TouliaoMessagingService 通知。
...
class MessageNotificationBridge @Inject constructor(
    ...
    /** 由 TouliaoApp.onCreate 调用一次：注册前后台追踪 + 开始监听 socket 新消息。 */
    ...
    companion object {
        // 全局前台标记：供 TouliaoMessagingService 判断 FCM 到达时 App 是否在前台，
