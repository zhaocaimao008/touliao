...
vi.mock('./url', () => ({ mediaUrl: vi.fn(url => url + '?ticket=fresh') }));
...
it('awaits real completion and publishes byte progress and save path', async () => {
  start(); expect(manager.getState('video').status).toBe('downloading');
...
it('HTTP failure remains failed and retry renews URL and task identity', async () => {
  const { mediaUrl } = await import('./url'); start();
  ...
  mediaUrl.mockReturnValueOnce('/uploads/video.mp4?ticket=renewed');
  manager.retryDownload('video'); expect(pending[1].args[0]).toContain('ticket=renewed');
...
it('cancellation reaches main process and waits for cleanup', async () => {
  start(); manager.cancelDownload('video');
...
it('IPC rejection and an old preload missing completion never claim success', async () => {
  start(); pending[0].reject(Error('IPC disconnected')); await tick();
  expect(manager.getState('video').status).toBe('failed');
  manager.retryDownload('video'); pending[1].resolve(undefined); await tick();
  expect(manager.getState('video').status).toBe('failed');
});
...
it('duplicate clicks and retry of active tasks never start a second request', async () => {
  start(); start(); manager.retryDownload('video'); expect(pending).toHaveLength(1);
  pending[0].resolve({ status: 'completed' }); await tick();
});
it('progress from another attempt does not change this task', async () => {
  start(); progress({ id: 'obsolete-id', status: 'downloading', progress: 99 });
