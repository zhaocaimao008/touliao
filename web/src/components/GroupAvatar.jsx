...
function GroupGridCell({ member = {}, cellSize }) {
  useMediaCredentials();
  const avatarUrl = mediaUrl(member.avatar);
...
export function GroupAvatar({ members = [], size = 48, avatar = '' }) {
  ...
  if (avatar && !avatarErr) {
    return <img src={avatarUrl} alt="" loading="lazy" onError={() => setAvatarErr(true)} style={{ width: size, height: size, borderRadius: Math.max(3, Math.round(size * 0.13)), objectFit: 'cover', flexShrink: 0 }} />;
  }
