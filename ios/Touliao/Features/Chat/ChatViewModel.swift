...
final class ChatViewModel: ObservableObject {
    ...
    /// 结构化 JSON 在 fileUrl(content 是人话,老客户端兜底直接显示)
    func callText(_ msg: Message) -> String {
        guard let data = msg.fileUrl.data(using: .utf8),
              let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else { return msg.content.isEmpty ? "通话结束" : msg.content }
        ...
        default:         return msg.content.isEmpty ? "通话结束" : msg.content
        }
    }
