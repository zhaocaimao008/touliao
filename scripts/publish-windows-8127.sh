...
[[ "$run_id" =~ ^[0-9]+-[0-9]+$ ]]
key="$RUNNER_TEMP/windows-8127-deploy-key"
umask 077
printf '%s\n' "$DEPLOY_SSH_KEY" > "$key"
...
node scripts/verify-windows-8127.cjs "$artifact" > "$evidence/verification.json"
ssh "${ssh_args[@]}" "$remote" "mkdir -p '$stage'"
for name in latest.yml latest.yml.sig touliao-8.1.27-setup.exe touliao-8.1.27-setup.exe.blockmap; do
  scp "${ssh_args[@]}" "$artifact/$name" "$remote:$stage/$name"
done
scp "${ssh_args[@]}" scripts/windows-8127-publication.json "$remote:$stage/spec.json"
...
verify_public() {
  url=$1
  expected=$2
  curl --fail --silent --show-error --retry 3 --max-time 240 "$url" -o "$RUNNER_TEMP/windows-8127-received"
  cmp "$expected" "$RUNNER_TEMP/windows-8127-received"
}
base=https://touliao.cc/downloads
verify_public "$base/updates/touliao-8.1.27-setup.exe" "$artifact/touliao-8.1.27-setup.exe"
verify_public "$base/updates/touliao-8.1.27-setup.exe.blockmap" "$artifact/touliao-8.1.27-setup.exe.blockmap"
...
verify_public "$base/touliao-windows-latest.exe" "$artifact/touliao-8.1.27-setup.exe"
verify_public "$base/touliao-windows-latest-setup.exe" "$artifact/touliao-8.1.27-setup.exe"
printf '%s\n' '{"publicFilesByteIdentical":true,"version":"8.1.27","platform":"windows"}' > "$evidence/public-verification.json"
