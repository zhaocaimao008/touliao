...
const spec = require('./windows-8127-publication.json');
...
assert.match(yml.toString(), /^version: 8\.1\.27$/m);
const exe = fs.readFileSync(path.join(dir, 'touliao-8.1.27-setup.exe'));
...
console.log(JSON.stringify({ version: spec.version, buildRun: spec.buildRun, sourceCommit: spec.buildCommit,
  sha256: spec.files['touliao-8.1.27-setup.exe'], bytes: exe.length, ed25519: true, authenticode: false }));
