...
> 最新 Windows 状态：8.1.27 已发布，真实 8.1.26 客户端完成生产更新发现、签名、下载、原生安装和登录/缓存保留测试；见 [自动更新发布记录](WINDOWS-AUTO-UPDATE-8.1.27-20260919.md)。这仍是应用内覆盖升级。Android、iOS 状态未被本次 Windows 发布改变。

> 2026-09-19 补充：用户要求单独出 Windows 安装包后，已生成 8.1.27 手动审阅包，并完成原生 Windows VM 的 8.1.26 → 8.1.27 覆盖升级、登录和消息缓存保留测试。无 Authenticode，未发布生产更新；详见 [新版安装包交付记录](WINDOWS-PACKAGE-8.1.27-20260919.md)。下文保留 9 月 18 日审计时的历史状态。
