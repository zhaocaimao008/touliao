...
- DS-023 dependency: Android CallState has no end-reason property; iOS exposes timeout/network only. UI cannot safely distinguish busy/declined without a change to protected call state producers. iOS CallManager disposes ended state after 800 ms (1800 ms for timeout). Android's 800 ms delay is in CallHost before consumeEnded(), not in its core; changing that UI delay alone would not supply the missing reasons. Neither timing was changed. Never fabricate a reason from elapsed time or strings.
