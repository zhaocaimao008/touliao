...
exports.updateSettings = asyncHandler(async (req, res) => {
  const result = svc.updateSettings(req.user.id, req.body);
  profileChanged(req.app.get('io'), req.user.id);
  res.json(result);
});
...
exports.getUserDetail = asyncHandler(async (req, res) => { res.setHeader('Cache-Control', 'private, no-store'); res.json(await svc.getUserDetail(req.user.id, req.params.id)); });
