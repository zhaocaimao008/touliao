...
exports.webSubscribe = asyncHandler(async (req, res) => {
  svc.webSubscribe(req.user.id, req.body.subscription, req.user.jti);
  res.json({ success: true });
});
exports.webUnsubscribe = asyncHandler(async (req, res) => {
  svc.webUnsubscribe(req.user.id, req.body.endpoint, req.user.jti);
  res.json({ success: true });
});
exports.saveDeviceToken = asyncHandler(async (req, res) => {
  svc.saveDeviceToken(req.user.id, req.body.token, req.body.platform, req.user.jti);
  res.json({ success: true });
});
exports.deleteDeviceToken = asyncHandler(async (req, res) => {
  svc.deleteDeviceToken(req.user.id, req.body.token, req.user.jti);
  res.json({ success: true });
});
