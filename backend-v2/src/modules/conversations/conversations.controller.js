...
exports.pin  = asyncHandler(async (req, res) => { await svc.setPinned(req.user.id, req.params.convId, req.body.pinned); invalidateSocial(io(req), [req.user.id], ['conversations']); res.json({ success: true }); });
exports.mute = asyncHandler(async (req, res) => { await svc.setMuted(req.user.id, req.params.convId, req.body.muted); invalidateSocial(io(req), [req.user.id], ['conversations']); res.json({ success: true }); });
// 会话归档/取消归档：仅本人自己的会话维度，不影响其他成员
exports.archive = asyncHandler(async (req, res) => { await svc.setArchived(req.user.id, req.params.convId, req.body.archived); invalidateSocial(io(req), [req.user.id], ['conversations']); res.json({ success: true }); });
