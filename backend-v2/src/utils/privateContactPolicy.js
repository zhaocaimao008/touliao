...
function privateContactDenial(senderId, recipientId) {
  ...
  const recipient = readDb.prepare('SELECT banned FROM users WHERE id=?').get(recipientId);
  const sender = readDb.prepare('SELECT banned FROM users WHERE id=?').get(senderId);
  if (!recipient || !sender || recipient.banned || sender.banned) return 'ACCOUNT_UNAVAILABLE';
