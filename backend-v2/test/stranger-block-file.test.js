...
describe('屏蔽陌生人消息：文本与文件一致拦截（round43 回归）', () => {
  ...
  test('陌生人发红包被拒（403，round45——红包也会推一条消息）', async () => {
    const res = await request(app).post('/api/messages/red-packet/send')
      .set('Authorization', `Bearer ${u1.token}`)
      .send({ conversationId, totalAmount: 10, totalCount: 1, greeting: '借红包骚扰' });
    expect(res.status).toBe(403);
    expect(res.body.error).toMatch(/屏蔽陌生人/);
  });
