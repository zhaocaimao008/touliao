...
describe('注册', () => {
  ...
  test('异常路径：缺失必填字段（无密码）→ 400，不应 500', async () => {
    const res = await request(app).post('/api/auth/register').send({ ...({ phone: '+86-13900000000' }), legalConsent: require('./legal-consent.cjs') });
    expect(res.status).toBe(400);
  });
});
...
describe('登录', () => {
  ...
  test('正常路径：正确手机号+密码登录成功', async () => {
    const res = await request(app).post('/api/auth/login').send({ ...({ phone: seededPhone, password: seededPassword }), legalConsent: require('./legal-consent.cjs') });
    expect(res.status).toBe(200);
    expect(res.body.token).toBeTruthy();
  });
  ...
  test('异常路径：手机号不存在 → 400，错误提示与"密码错误"一致（防枚举）', async () => {
    const res = await request(app).post('/api/auth/login').send({ ...({ phone: '+86-13000000099', password: 'whatever123' }), legalConsent: require('./legal-consent.cjs') });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('手机号或密码错误');
  });
