...
const SERVER_URL = argValue('--vxin-server-url=') || 'https://touliao.cc';
...
const electronAPI = {
  ...
  getUpdateKeyStatus: ()  => ipcRenderer.invoke('update:getKeyStatus'),

  // 独立下载任务：等待真实落盘，并按任务 ID 接收进度 / 取消。
  downloadFile:     (url, filename, autoOpen, id) => ipcRenderer.invoke('file:download', { url, filename, autoOpen, id }),
  cancelDownload:   (id) => ipcRenderer.invoke('file:cancelDownload', id),
